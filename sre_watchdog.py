...
JENKINS_TOKEN = "11a5990682a42898140223a5a21b5aa8d3"  # Update this
    ...
    for job in jobs:
        try:
            ...
            if build_result == "FAILURE":
                print(f"\n🚨 [WATCHDOG] Detected new failure on {job} (Build #{build_num}).")
                
                # --- FIX 1: OPTIMISTIC LOCKING ---
                # Mark it as processed IMMEDIATELY before calling the AI.
                ...
                # --- FIX 2: LIVE OUTPUT & SIMULTANEOUS CAPTURE ---
                process = subprocess.Popen(
                    [sys.executable, script_path, temp_log], 
                ...
                for line in iter(process.stdout.readline, ''):
                    ...
                    full_output += line # Accumulate for the text file
                ...
                print(f"\n✅ [WATCHDOG] SRE Remediation complete for {job}. Report saved.")
                
                # Clean up temp file
                if os.path.exists(temp_log): os.remove(temp_log)
                
                # --- FIX 3: JOB COOLDOWN ---
                # Let the API breathe before moving to the next job in the loop
                print("⏳ Resting for 20 seconds before checking next job...")
                time.sleep(20)
...
if __name__ == "__main__":
    ...
    print("Polling Jenkins every 30 seconds...")
    print("========================================")
    while True:
        check_for_failures()
...
#                 # Run the Virtual SRE Engine
#                 script_path = os.path.join(os.path.dirname(__file__), "virtual_sre.py")
#                 result = subprocess.run([sys.executable, script_path, temp_log], capture_output=True, text=True)
