...
import re
import json
from jenkins_connector import get_latest_failed_log
...
with col1:
    ...
    if st.button("Trigger Virtual SRE Auto-Remediation", type="primary"):
        with st.spinner(f"Fetching live logs from Jenkins and analyzing..."):
            live_log_text = get_latest_failed_log("Billing-Service-Core")
            temp_log_path = os.path.join(os.path.dirname(__file__), "live_jenkins_log.txt")
            with open(temp_log_path, "w", encoding="utf-8") as f:
                f.write(live_log_text)
            ...
            result = subprocess.run(
                ["python", script_path, temp_log_path],
                capture_output=True,
                text=True,
                env=env
            )
