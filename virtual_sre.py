    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
                ...
                types.Tool(
                    function_declarations=[
                        ...
                        types.FunctionDeclaration(
                            ...
                            description="A fallback tool to execute an arbitrary bash/shell command on the build agent.",
            ...
            for step in range(8): # Max 8 turns to prevent infinite loops
                # --- CAPPED RETRY LOGIC FOR API ISSUES ---
                max_retries = 3
                attempt = 0
                ...
                while True:
                    try:
                        ...
                        # Catch 429 Rate Limits AND 503 Unavailable spikes
                        if any(err in error_msg for err in ["429", "quota", "exhausted", "503", "unavailable"]):
                            attempt += 1
                            if attempt > max_retries:
                                print(f"\n[ERROR] API failed after {max_retries} attempts: {str(e)}")
                                return # Cleanly exit the MCP connection
                                
                            print(f"\n[!] API Busy (Attempt {attempt}/{max_retries}). SRE taking a 15-second breather...")
                ...
                if response.function_calls:
                    ...
                    # 3. Feed the MCP results back to Gemini
...
def extract_bamboo_metadata(log_text: str):
    ...
    for line in log_text.splitlines()[:20]:
        ...
        if prepare_match:
            ...
            metadata["plan_key"] = prepare_match.group(2) 
            metadata["agent_id"] = prepare_match.group(3) 
            break 
